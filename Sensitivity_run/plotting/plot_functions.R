plot_contour<-function(x,y,data,var_name,cbar_limit,ii,customize_colorbar){
    with_legend = FALSE
    if(missing(customize_colorbar)){
      lcolor = "Greens"
      customize_colorbar = FALSE 
    }else{
...
#    scale_fill_gradient2(low="blue", mid="white", high="red", midpoint=0,limits = cbar_limit)
    if(customize_colorbar){
       v<-v+scale_fill_gradientn(name=var_name,colours = c("blue","white","red"), 
                             values = rescale(c(cbar_limit[1],0,cbar_limit[2])),
                             guide = "colorbar", limits=cbar_limit)
    }else{
       v<-v+scale_fill_distiller(name=var_name,palette = lcolor,limits = cbar_limit,direction = 1)
    }
#remove axis labels 
...
#             axis.ticks.x=element_blank(),
    ...
    print(ynames[i])
    df400=df[df$CO2==800,]
    print(mean((df400$EXP-df400$CTL)/df400$CTL*100))
    if(i==1){
       p<-ggplot(df)+
...
timeseries_plot_Rd<-function(up,bot,pdfname){
  df1 <- within(up, CO2 <- factor(CO2,levels=names(sort(table(CO2),decreasing=FALSE))))
  df2 <- within(bot, CO2 <- factor(CO2,levels=names(sort(table(CO2),decreasing=FALSE))))
  CO2_label = paste(unique(df1$CO2),"ppm")
  plot_list = list()
  ynames = colnames(df1)[-c(1,2)]#remove year and CO2 
  text_size = 24
 for (i in 1:length(ynames)){
  df = data.frame(Years=df1$year,up=df1[,ynames[i]],bot=df2[,ynames[i]],CO2=df1$CO2)
  y_label = bquote(Delta*.(ynames[i])~('%'))
  ylimit = c(0,15)
  tag_label = paste0("(",letters[i],")") 
  print(ynames[i])
  if(i==1){
     p<-ggplot(df)+
        geom_line(aes(x=Years,y=up/bot*100,color=CO2),size=2)+
        geom_point(aes(x=Years,y=up/bot*100,color=CO2))+
        scale_x_continuous(breaks = unique(df$Years),guide = guide_axis(angle = 45))+
        scale_y_continuous(limits = ylimit,labels = label_number(accuracy = 1))+
        ylab(y_label) + 
        labs(tag = tag_label)+
...
#              axis.ticks.x=element_blank(),
 ...
 grid.arrange(grobs = grobs,nrow=2,ncol=2)
 dev.off()
}
