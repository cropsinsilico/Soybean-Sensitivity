...
for (ii in 1:length(CO2s)){
  ...
  method  = 3 #1: percentage %; 2: absolute difference; 3: no difference
  ...
  for (j in 1:length(var2plot)){
      ...
      #  print(paste("at CO2",CO2_case,varname,"ranges",range(var_diff_avg)[1],range(var_diff_avg)[2]))
      #  print(paste("control is",var_diff_avg[v0_ind,j0_ind]))
      pos_ind = which(var_diff_avg>0,arr.ind=TRUE)
      vj = cbind(v_scaler[pos_ind[,1]],j_scaler[pos_ind[,2]]) - 1
      vj_multi = vj[,1] * vj[,2]
      vj_plus  = vj[,1] + vj[,2]
      fig_i = plot_contour(v_scaler,j_scaler,var_diff_avg,varname1,cbar_limit,ii)
...
#heatmap plot
pdf(paste0(output_folder,"Fig1.pdf"),height = 30, width=24)
